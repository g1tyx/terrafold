function View() {
    ...
    this.updatePopulation = function() {
        ...
        document.getElementById('scienceRatio').innerHTML = game.population.scienceRatio+"% 科学";
        document.getElementById('scienceRatio').innerHTML = game.population.scienceRatio < 50 ? 100-game.population.scienceRatio+"% 科学" : game.population.scienceRatio+"% 金钱";
    ...
    this.updateComputerRowProgress = function() {
        ...
        for(var i = 0; i < game.computer.processes.length; i++) {
            ...
            if(row.cost !== 0) {
                ...
                document.getElementById(baseId+"Cost").innerHTML = "每轮消耗 "+intToString(row.cost) + " "+cnItem(row.costType);
            } else {
    ...
    this.addComputerRow = function(dataPos) {
        ...
        var tooltip = "<div id='"+baseId+"CurrentTicks'></div> 轮<br>" +
            "<div id='"+baseId+"TicksNeeded'></div> 轮需要<br>" +
    ...
    this.updateRobotsRowProgress = function() {
        ...
        for(var i = 0; i < game.robots.jobs.length; i++) {
            ...
            if(row.cost) {
                document.getElementById(baseId+"Cost").style.display = "block";
                var costString = "每轮消耗 "+intToString(row.cost[0]) + " "+cnItem(row.costType[0]);
                costString += row.cost.length > 1 ? " 和 " + intToString(row.cost[1]) + " " + cnItem(row.costType[1]) : "";
    ...
    this.updateSpaceStation = function() {
        ...
        for(var i = 0; i < game.spaceStation.orbiting.length; i++) {
            orbitString += intToString(game.spaceStation.orbiting[i].amount) + " " + cnItem(game.spaceStation.orbiting[i].type);
            orbitSendString += intToString(game.spaceStation.orbiting[i].amount / 10000, 4) + " " + cnItem(game.spaceStation.orbiting[i].type);
    ...
    this.updateTractorBeam = function() {
        ...
        for(var i = 0; i < comets.length; i++) {
            text += cnItem(comets[i].name) + " 和 " +
                intToString(comets[i].amount, 1) +
                " " + cnItem(comets[i].amountType) +
                " 传递 " + comets[i].duration + "<br>";
    ...
    this.updateHangar = function() {
        document.getElementById("hangar0Sending").innerHTML = game.hangars[0].sendRate + " 在 "+round1(game.hangars[0].timeRemaining/10) +" 秒内。";
    };
